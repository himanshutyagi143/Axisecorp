@section('meta')
	...
	<meta name="keywords" content="{{empty($posts->meta_keyword) ? 'Axis Ecorp News' : $posts->meta_keyword}}"/>
	...
	<meta property="og:url" content="{{url('/news/'.$posts->slug)}}" />
...
@section('news-current-menu', 'current-menu-item')
			...
			<div id="breadcrumbs">
				<div class="breadcrumbs-inner">
					<div class="breadcrumb-trail">
						...
						@else
						<span class="sep">|</span>
						<span class="trail-end"><a href="/news">News</a></span>
						<span class="sep">|</span>
		...
		<div id="site-content" class="site-content clearfix">
			<div id="inner-content" class="inner-content-wrap">
				<article class="hentry data-effect">
						...
						@if($is_slug==1) <img src="{{$posts->image}}" alt="" class="img-responsive center-block"> @endif
						<div class="post-calendar">
							<span class="inner">
								<span class="entry-calendar">
									<span class="day">{{date('d',strtotime($posts->blog_date))}}</span>
									<span class="month">{{date('M',strtotime($posts->blog_date))}}</span>
								</span>
							</span>
						</div>
					...
					<div class="post-content-wrap clearfix">
						<h2 class="post-title">
							<span class="post-title-inner">
								@if($is_slug==1) <h2>{{$posts->title}}</h2> @else <a href="/news/{{$posts->slug}}">
									<h2>{{$posts->title}}</h2>
								</a>@endif
							</span>
						</h2>
                  ...
                  @if($posts->blog_date)
                  {{date('M d, Y',strtotime($posts->blog_date))}}
                  @endif
                  </span></span></span>
									@if($posts->author)<span class="post-by-author item"><span class="inner"><a href="#">By: {{$posts->author}}</a></span></span>@endif
									@if($posts->source_link != null)
										<span class="comment item"><span class="inner">Source Link: <a href="{{$posts->source_link}}" target="_blank">Click Here </a></span></span>
									@endif
								</div>
							</div>
						</div>
						<div class="post-content post-excerpt margin-bottom-43">
							@if($is_slug!=1) <p class="line-height-27 letter-spacing-005">{{str_limit(strip_tags($posts->content),400,'...')}}</p>@endif
							@if($is_slug==1) <div class="long_text">{!!$posts->content!!}</div>@endif
						</div>
						<div class="post-socials detail_sociail">
							<a target="_blank" style="background:#25D366"
							   href="https://api.whatsapp.com/send?text={{urlencode(url('/news/'.$posts->slug.'/'))}}"
							   data-action="share/whatsapp/share"><span class="fa fa-whatsapp"></span></a>
							<div class="" data-href="{{url('/news/'.$posts->slug)}}"><a target="_blank" href="https://www.facebook.com/sharer/sharer.php?u={{urlencode(url('/news/'.$posts->slug.'/'))}}&amp;src=sdkpreparse" class="fb-xfbml-parse-ignore"><span class="fa fa-facebook-square"></span></a></div>
							<a href="https://api.addthis.com/oexchange/0.8/forward/twitter/offer?url={{urlencode(url('/news/'.$posts->slug.'/'))}}" class="twitter" target="_blank"><span class="fa fa-twitter"></span></a>
							<a href="https://api.addthis.com/oexchange/0.8/forward/linkedin/offer?url={{urlencode(url('/news/'.$posts->slug.'/'))}}" class="linkedin" target="_blank"><span class="fa fa-linkedin-square"></span></a>
						{{-- <a href="https://api.addthis.com/oexchange/0.8/forward/instagram/offer?url={{urlencode(url('/news/'.$posts->slug.'/'))}}" class="pinterest" target="_blank"><span class="fa fa-instagram"></span></a>--}}
						</div>
						<div class="clearfix"></div>

						<div class="post-tags-socials clearfix" style="margin-bottom:20px !important;">
							<div class="post-tags detail_post_tag ">
								@if(count($posts->tags()->get()) > 0)
								<span>Tags :</span>
								@foreach($posts->tags()->get() as $tag)
								<a href="/news/tag/{{$tag->tagName}}">{{$tag->tagName}}</a>
								@endforeach
								@endif
							</div>
						</div>
		...
		<div id="sidebar">
			...
			<div id="inner-sidebar" class="inner-content-wrap">
				<div class="widget widget_search">
					<form action="/news/search" method="post" class="search-form style-1">
						{{csrf_field()}}
					...
					<ul class="lastest-posts data-effect clearfix">
						...
						<li class="clearfix">
							<div class="thumb data-effect-item has-effect-icon">
								...
								<div class="elm-link">
									<a href="/news/{{$recentPost->slug}}" class="icon-2"></a>
								</div>
							</div>
							<div class="text">
								<h3><a href="/news/{{$recentPost->slug}}">{{$recentPost->title}}</a></h3>
				...
				<div class="widget widget_tags margin-top-55">
					...
					<div class="tags-list">
						...
						<a href="/news/category/{{$category->categorySlug}}">{{$category->categoryName}}</a>
						@endforeach
					</div>
				</div>
