   ...
   <!--<script src="/assets/payform/js/jquery-3.3.1.min.js"></script>-->
    ...
    <body>
        <div class="wrapper">
            ...
            <form action="/booknowsubmit" id="wizard" class="wizard" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-row">
						...
						<div class="form-col">
                            ...
                            <div class="form-holder">
                                ...
                                <input type="text" name="zip" class="form-control" id="pincode" onchange="getPinCodeData(this.value)" value="" required>
                            </div>
                ...
                <section id="sectiontwo" style="display:none;">
                    ...
                    <div class="form-group payment_plan_div" id="payment_plan_div" style="display:none;">
                        <label class="form-label" style="color:#3377c0;"></label>
		...
		$.ajax({
			...
			success: function (data) {
				...
				if (data.message == 'success')
				{
					...
					for (i = 0; i < unitDetails.length; i++) {
						if (unitDetails[i].value != null) {
							...
							if (unitDetails[i].input_type != "file") {
								...
								if(unitDetails[i].name != "total_unit_cost" && unitDetails[i].name != "basic_sale_price"){
					...
					unitData = unitData + '<tr><td>Basic Sale Price</td><td><span>' + data.basic_sale_price + ' /- </span> </td></tr>';
					for(j = 0; j< totalChargeComponent.length; j++){
						 unitData = unitData + '<tr><td>' + totalChargeComponent[j].charge_component_name + '</td><td><span>' + totalChargeComponent[j].charge_component_price + ' /- </span> </td></tr>';
					}
					unitData = unitData + '<tr><td><b> Total Unit Cost </b></td><td><span>' + data.total_unit_cost + ' /- </span> </td></tr>';
		...
		getfrontendPaymentPlan(unit_id);
                        ...
                        function proceedToPay() {
                            $('.template_div').each(function () {
                                ...
                                //console.log($('#templet_html_'+id).val());
                            });
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    ...
                                    if (data.message == 'success')
                                    {
                                        ...
                                        var amount = [];
                                        var grand_total = 0;
                                        //console.log(planComponentDetails);
