@extends('frontend.master')

@section('content')
@include('frontend.profile.upper_link')

<div class="about_inner">
<div class="banner-overlay">
<div class="banner corporate_banner">
<div class="col-md-12">
<h1 class="about_title">Our Services </h1>
<span>


</span>

</div></div></div>



</div>
<div class="container">
	<div class="row">
	
	<div class="col-md-12 ">
                     <div class="w3l-grids-about">
                        <div class="w3ls-agile-left ">
                           <p>Axis Blues is planned for prime hospitality &amp; rainforest resort destination, its aim is to Eco-Luxury who preserves the excitement of nature living with modern comfort.</p>
                   
                           <p>The experience is made flawless by an innovative service concept, informal yet impeccable. The recreational area and restaurant, accompanied by exclusive function spaces, wants to become the most coveted destination point for nature lovers.</p>
                           <p>Laze around on dew-laced nature &amp; lush green views that melt your hart; fresh air wake up along with birds and bees, the scenic backdrop mountain, the quaint nature &amp; it’s simple smiling inhabitants welcomes you to revel in a vibrant &amp; harmonious ecosystem with class and high standard service and comfort with contemporary modern comfort, enriched by designer furniture &amp; materials. </p>
                        </div>
                        <div class="clearfix"> </div>
                        <div class="services" id="services">
                           <!-- 728x90 -->
                           <div class="row">
                              <div class="col-md-12">
                                 <div class="Highlights_section ">
                                    <h3> Highlights</h3>
                                 </div>
                              </div>
                           </div>
                           <div class="row">
                              <div class="col-md-3  col-sm-3">
                                 <div class="Highlight">
                                    <img class="font-icons" src="fronted/img/rera.jpg" alt="Rera logo">
                                    <b>Unit Registry </b>
                                 </div>
                              </div>
                              <div class="col-md-3 col-sm-3">
                                 <div class="Highlight">
                                    <img class="font-icons" src="fronted/img/planning.png" alt="Image 3">
                                    <b>Commercial Approved </b>
                                 </div>
                              </div>
                              <div class="col-md-3 col-sm-3">
                                 <div class="Highlight">
                                    <img class="font-icons" src="fronted/img/tax.png" alt="Tax image 3">
                                    <b>CLP </b>
                                 </div>
                              </div>
                              <div class="col-md-3 col-sm-3">
                                 <div class="Highlight">
                                    <img class="font-icons" src="fronted/img/map.png" alt="Map ">
                                    <b>Smart Hotel Suites</b>
                                 </div>
                              </div>
                              <div class="col-md-3 col-sm-3">
                                 <div class="Highlight">
                                    <img class="font-icons" src="fronted/img/building.png" alt="Building">
                                    <b>Ground + 3 </b>
                                 </div>
                              </div>
                              <div class="col-md-3 col-sm-3">
                                 <div class="Highlight">
                                    <img class="font-icons" src="fronted/img/award.png" alt="award">
                                    <b>100% Vastu Complaince </b>
                                 </div>
                              </div>
                              <div class="col-md-3 col-sm-3">
                                 <div class="Highlight">
                                    <img class="font-icons" src="fronted/img/apartment.png" alt="Apartment 2">
                                    <b>Rental + stay </b>
                                 </div>
                              </div>
                              <div class="col-md-3 col-sm-3">
                                 <div class="Highlight">
                                    <img class="font-icons" src="fronted/img/kitchen.png" alt="Kitchen ">
                                    <b>Fully Funrished </b>
                                 </div>
                              </div>
                           </div>
                        </div>
                     </div>
                  </div>
				
</div>
</div>


@stop