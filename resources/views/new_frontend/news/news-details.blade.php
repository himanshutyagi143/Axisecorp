                ...
                @if(isset($post_exits) && $post_exits != 0)
                    ...
                    <article class="post">
                       <div class="post-thumbnail">
                          <img alt="{{$posts->image_name}}" class="img-responsive" src="{{$posts->image}}">
						              @if($posts->news_city != null) <span>{{$posts->news_city}}</span> @endif
                       </div>
                       ...
                       <ul class="social-list">
                          <li><a href="https://api.whatsapp.com/send?text={{urlencode(url('/news/'.$posts->slug.'/'))}}"  data-action="share/whatsapp/share" class="fa fa-whatsapp whatsapp" target="_blank"></a></li>
                          <li><a href="https://api.addthis.com/oexchange/0.8/forward/twitter/offer?url={{urlencode(url('/news/'.$posts->slug.'/'))}}" class="fa fa-twitter twitter" target="_blank"></a></li>
                          <li><a href="https://api.addthis.com/oexchange/0.8/forward/linkedin/offer?url={{urlencode(url('/news/'.$posts->slug.'/'))}}" class="fa fa-linkedin linkdin" target="_blank"></a></li>
                          <li><a href="https://www.facebook.com/sharer/sharer.php?u={{urlencode(url('/news/'.$posts->slug.'/'))}}&amp;src=sdkpreparse" class="fa fa-facebook facebook" target="_blank"></a></li>
                       </ul>
                       <div class="blog-tags">
                        @if($posts->tags)
                       ...
                       <div class="subscribe-form">
                        ...
                        <div class="alert alert-success alert-block text-center show_msg_block" style="display: none;">
                          ...
                          <h4>Please fill form to subscribe news!</h4>
                        ...
                        @foreach($recent as $recentpost)
                         <article class="recent-post">
                            <div class="recent-post-thumbnail">
                               <a href="/news/{{$recentpost->slug}}"><img alt="{{$recentpost->image_name}}" src="{{$recentpost->image}}" class="wp-post-image"></a>
                               @if($recentpost->city) <span>{{$recentpost->city}}</span> @endif
                            </div>
                 ...
                 @else
                  <div class="alert alert-danger">
                     <h4 class="blognotavail">News Not Available</h4>
                  </div>
                  @endif 
              </div>
           </div>
