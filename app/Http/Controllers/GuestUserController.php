...
class GuestUserController extends Controller
{
	...
	public function unitchecker(request $request)
	{
		$a=$request->all();
		dd($a);
		/*$projetBlock = DB::table('unit_details')->join('project_blocks', function ($on) {
        ...
        return response()->json(['status' => 200, 'message' => 'success', 'projetBlock' => $projetBlock]);*/
	}
    ...
    public function yogvillaPayment($tab = 0, $token = '')
    {
        $projetBlock = DB::table('unit_details')->join('project_blocks', function ($on) {
         ...
         /*$soled=UnitDetails::where('project_id', '=', 11)
											->where('status', '=', 1)
                                            ->where('booking_status', '=', 2)->get();*/
         //dd($soled);
        /*foreach ($soled as $key => $value) 
        {
        	$soledunit[] = substr($value->unit_name, -2);
         ...
         foreach ($projetBlock as $key => $v1)
        {
         ...
         //$newArray[] = array_diff($soled,$unit);
        //dd($projetBlock);
        if ($token != '') {
            $id = Crypt::decrypt($token);
            $guest = DB::table('guest_user')->where('guest_user_id', '=', $id)->where('is_active', '=', 1)->first();
		...
		try {
			...
			$validator->after(function ($validator) use ($request, $project_id) {
				...
				if (!$checkBlockFloorUnit) {
					...
					if($request->unit_id != substr($checkBlockFloorUnit->unit,-2)){
    ...
    public function submitYogvillaPayment(Request $request)
    {
        try {
            ...
            $unit_no = (int)($request->unit_no);
		    $projetBlock = DB::table('unit_details')->join('project_blocks', function ($on) {
		    $on->on('project_blocks.block_id', '=', 'unit_details.block_id')->where('project_blocks.status', '=', 1);
			})->where('unit_details.project_id', '=', $project_id)
		    ->where('unit_details.status', '=', 1)
		    ->where('unit_details.booking_status', '=', 1)
		    ->select('unit_details.unit_name','project_blocks.block_name', 'project_blocks.block_id')->get();
		    
		    
			  $count=count($projetBlock);
			  /*for ($i=1; $i <=$count ; $i++) 
			  { 
...
//dd($projetBlock);
			    ...
			    foreach ($projetBlock as $key => $value) 
			    {
			    	$unit[] = (int)substr($value->unit_name, -2);
			    	
			    }
			    $array[] = array_combine(range(1, count($unit)), array_values($unit));
			    //dd($array[0]);
			    $unitid=array_search($unit_no , $array[0]);
			    //dd($unitid);
				if(!$unitid)
			   	{
			   		$validator->errors()->add('unit_no', 'Unit not Available');
			   	}
			   	//dd($unit_code);
			});
			$unit_no = (int)($request->unit_no);
		    ...
		    ->select('unit_details.unit_name','project_blocks.block_name', 'project_blocks.block_id')->get();
			    ...
			    foreach ($projetBlock as $key => $value) {
			    	if($key<9)
			    	{
			    		$unit[] = substr($value->unit_name, -1);
			    	}
			    	else
			    	{
			    		$unit[] = substr($value->unit_name, -2);
			    	}
			    	
			    }
			    $unitid=array_search($unit_no , $unit);
				if(!$unitid)
			   	{
			   		$validator->errors()->add('unit_no', 'Unit not Available');
			   	}
			   	$unit_code=$projetBlock[$unitid]->unit_name;
			   	
			//dd($unit_code);
		   	$checkBlockFloorUnit = DB::table('unit_details')->where('unit_name', '=', $unit_code)->where('project_id', '=', $project_id)->first();
		   	$unit_idd = $checkBlockFloorUnit->id;
           ...
           if ($validator->fails()) {
            ...
            $mail = Mail::send('emails.unitBooking', ['data' => $data], function ($m) use ($data) {
                $m->from($data['from'], 'axisecorp');
                $m->to($data['to'])->subject($data['subject']);
                $m->setBody($data['msg'], 'text/html');
            });
            ...
            $guestId = DB::table('guest_user')->insertGetId(['first_name' => $request->first_name, 'last_name' => $request->last_name, 'email' => $request->email, 'phone' => $request->phone, 'unit_id' => $unit_idd, 'amount' => $totalAmount, 'project_id' => $project_id, 'is_active' => 1, 'block_id' => '', 'otp' => $otp, 'created_at' => date('Y-m-d'), 'passport_media_id' => $passport_media_id, 'pan_card_media_id' => $pan_card_media_id,'sales_representative'=>$request->SalesRepresentative,'sales_representative_phone'=>$request->SalesRepresentativePhone]);
    ...
    public function verifyYogUnit($token)
    {
        try {
            ...
            if(substr($unitDetail->unit,-3, 1)==0) {
                $unitDetail->villa_type = "A";
            } elseif(substr($unitDetail->unit,-3, 1)==1){
                $unitDetail->villa_type = "B";
            }elseif(substr($unitDetail->unit,-3, 1)==2){
                $unitDetail->villa_type = "C";
            }


            if ($guestUser) {
                return view('yogvillapayments.verifyYogUnit', ['guestUser' => $guestUser, 'tokenencypty' => $token, 'unitDetail' => $unitDetail, 'project' => $project]);
            } else {
                return redirect('/yogvilla-payment');
            }
        } catch (Exception $e) {
    ...
    public function payForYogBooking($guest_token)
    {
        //echo 'ddd'; exit;
        try {
            if ($guest_token) {
                ...
                $temp_customer_id = TempCustomer::insertGetId([
                    'sender_id' => 1,
                    'company_type_id' => 1,
                    'block_id' => $unitdetailsArr->block_id,
                    'unit_id' => $unit_id,
                    'project_id' => $project_id,
                    'customer_code' => $customer_code,
                    'first_name' => $guestUserArray->first_name,
    ...
    public function verifyOtpForYogBooking($tokenencypty)
    {
        ...
        try {
            ...
            if ($guestUser) {
                $unitDetail = DB::table('floor_unit')->where('id', '=', $guestUser->unit_id)->where('project_id', '=', $guestUser->project_id)->where('status', '=', 1)->first();
                return view('yogvillapayments.verify_otp_for_yog_booking', ['guestUser' => $guestUser, 'tokenencypty' => $tokenencypty, 'unitDetail' => $unitDetail]);
            } else {
                return redirect('/yogvilla-payment');
            }
        } catch (Exception $e) {
    ...
    public function postverifyOtpForYogbooking(Request $request)
    {
        ...
        return redirect('/payForYogBooking/' . $request->guest_token);
        // $this->payForBooking($guestUser);
    }
	...
	public function paymentStatus($order_id){
		...
		{
			DB::table('tbl_link')->where('customer_code','=',$orderData->customer_code)->where('status','=',4)->update(['status' => 2]);
		}
